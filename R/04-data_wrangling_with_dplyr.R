...
jfkFlights <- filter(flights, DayOfWeek == 6, Dest == "JFK")

# 2) Combine the Year, Month and DayofMonth variables to create a Date column

jfkFlights <- mutate(jfkFlights, Date = as.Date(paste(Year,Month,DayOfWeek,sep="-")))

# 3) Result:
select(jfkFlights,Date)
...
summarise(aa, n_flights=n(),n_canc = sum(Cancelled),p_canc = 100*(n_canc/n_flights), avg_delay = mean(ArrDelay,na.rm=TRUE))
...
summarise(aa, n_security = sum(CancellationCode=='D',na.rm=TRUE))

rm(aa)

# %>% OPERATOR ----------------------------------------------------------------------
# the pipe command is in the library magicpr
# Piping (Ctrl + Shift + M)
...
#You can use View() at the end of the pipe to check in a table out ourput
...
flights %>% 
  summarise(n=sum(ArrTime<DepTime,na.rm=TRUE))

##Check

flights %>% 
  filter(!is.na(ArrTime)&!is.na(DepTime)) %>% 
  summarise(n=sum(ArrTime<DepTime))
...
flights %>% 
  ...
  group_by(UniqueCarrier) %>% 
  summarise(avg = round(mean(ArrDelay))) %>% 
  mutate(rank = round(rank(avg),digits = 0)) %>% 
  arrange(rank)
...
foo <- flights %>% 
  mutate(CancellationCode = na_if(CancellationCode, "")) ## Transform the values by NAs
